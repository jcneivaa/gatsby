...
class Juniper extends React.Component {
    ...
    static defaultProps = {
        ...
        msgButton: 'run',
        msgLoading: 'Cargando...',
        msgError: 'Conexión fallida. Por favor recargar e intenta nuevamente.',
        classNames: {
            cell: 'juniper-cell',
            input: 'juniper-input',
    ...
    execute(outputArea, code) {
        ...
        const action = !this.state.fromStorage ? 'Iniciando' : 'Reconectando con el'
        outputArea.model.clear()
        outputArea.model.add({
            output_type: 'stream',
            name: 'stdout',
            text: `${action} Docker container en ${url}...`,
        })
